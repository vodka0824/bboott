...
const Click108Crawler = require('../utils/click108_crawler');
...
async function getHoroscope(signName, type = 'daily') {
    ...
    // 2. Real-time Crawl (Click108 with Puppeteer)
    console.log(`[Horoscope] Cache MISS, crawling with Puppeteer: ${cacheKey}`);
    try {
        const data = await Click108Crawler.fetchSignData(signName, type);
...
async function prefetchAll(type = 'daily') {
    ...
    console.log(`[Prefetch] Starting Puppeteer fetch for 12 signs (${type})...`);
    ...
    await Click108Crawler.closeBrowser();
...
function buildHoroscopeFlex(data, type = 'daily') {
    ...
    if (data.content) {
        bodyContents.push(flexUtils.createBox('vertical', [
            flexUtils.createText({
                ...
                wrap: true,
                lineSpacing: '5px' // This simulates spacing by wrapping text natively
            })
        ], {
    ...
    // 2. Lucky Grid (Only show if data is available, daily only)
    if (data.luckyNumber && data.luckyNumber !== 'N/A') {
        const luckyRows = [];
        ...
        if (data.luckyDirection && data.luckyDirection !== 'N/A') {
            luckyRows.push(flexUtils.createBox('horizontal', [
        ...
        bodyContents.push(flexUtils.createText({ text: '幸運指標', size: 'xs', weight: 'bold', color: '#999999', margin: 'md' }));
    ...
    if (data.stars && Object.keys(data.stars).length > 0) {
        ...
        map.forEach(m => {
            if (data.stars[m.key] !== undefined) {
                ...
                if (stars.length > 0) stars[0].margin = 'md';

                starBox.push(flexUtils.createBox('baseline', [
                    flexUtils.createText({ text: m.label + ' ', size: 'sm', color: '#555555', weight: 'bold', flex: 0 }),
                    ...stars
                ], { margin: 'md' }));
            }
        });

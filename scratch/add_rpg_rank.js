...
let rpgCode = fs.readFileSync(rpgPath, 'utf8');

const handleRpgRankCode = `
/**
...
async function handleRpgRank(context) {
    ...
    try {
        ...
        playersSnapshot.forEach(doc => {
            ...
            if (!displayName) return; // 忽略沒有名字的玩家
            ...
            const cp = Math.floor(atk * 2.5 + def * 2 + eva * 15 + crit * 15 + luk * 10 + pen * 15);
...
if (!rpgCode.includes('handleRpgRank')) {
    rpgCode = rpgCode.replace('module.exports = {', handleRpgRankCode + '\nmodule.exports = {');
    rpgCode = rpgCode.replace('addExp\n};', 'addExp,\n    handleRpgRank\n};');
    fs.writeFileSync(rpgPath, rpgCode, 'utf8');
    console.log('Appended handleRpgRank to rpg.js');
} else {
    console.log('handleRpgRank already exists in rpg.js');
}

...
const TOXIC_RESPONSES = [
    ...
    (city) => `你可以不要那麼無知嗎？${city}不歸台灣管，回去重讀地理吧。`,
    (city) => `系統偵測到用戶智商過低，無法處理查詢${city}的請求。`,
    (city) => `你是不是覺得自己很幽默？查${city}幹嘛？`,
    (city) => `要查${city}去問你媽，她應該比我清楚你為什麼會問這種蠢問題。`,
    (city) => `Error 404: Your Brain Not Found while searching for ${city}.`,
    (city) => `本機器人拒絕回答${city}的天氣，因為這侮辱了我的處理器。`,
    (city) => `施主，回頭是岸。${city}的天氣不是凡人可以窺探的。`,
    (city) => `你再問一次${city}試試看？信不信我直接登出給你看？`,
    (city) => `人生很短，不要浪費時間查${city}這種沒意義的東西。`,
    (city) => `你的地理老師現在應該在哭泣。${city}在哪以你的智商可能無法理解。`
];

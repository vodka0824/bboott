...
for (let i = lines.length - 1; i >= 0; i--) {
    if (lines[i].includes('equipment.js')) {
        ...
        if (obj.type === 'VIEW_FILE' && obj.content && (obj.content.includes('handlers/equipment.js') || obj.content.includes('handlers\\\\equipment.js'))) {
            ...
            if (codeLines.length > 0 && codeLines[0].includes('require')) {
                fs.writeFileSync(`scratch/recovered_eq_${obj.step_index}.js`, codeLines.join('\n'));
                console.log(`Saved scratch/recovered_eq_${obj.step_index}.js`);
                process.exit(0);
            }
        }
    }

...
const startStr = 'async function handleRpgRank(context) {';
const endStr = '\n    } catch (e) {';

if (code.includes(startStr)) {
    const newHandleRpgRank = `async function handleRpgRank(context) {
    ...
    const catchIdx = code.indexOf(endStr, startIdx);
    
    if (startIdx !== -1 && catchIdx !== -1) {
        code = code.substring(0, startIdx) + newHandleRpgRank + code.substring(catchIdx);
        fs.writeFileSync(rpgPath, code, 'utf8');
        console.log('Replaced handleRpgRank logic successfully.');
    } else {
        console.log('Could not find the function boundaries.');
    }
} else {
    console.log('handleRpgRank not found.');
}

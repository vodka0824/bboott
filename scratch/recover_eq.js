    ...
    if (lines[i].includes('equipment.js') && lines[i].includes('CodeContent')) {
        ...
        if (obj.tool_calls) {
            for (const call of obj.tool_calls) {
                if (call.name === 'write_to_file' && call.args && call.args.TargetFile && call.args.TargetFile.includes('equipment.js')) {
                    fs.writeFileSync('scratch/recovered_equipment.js', call.args.CodeContent);

...
const filePath = path.join(__dirname, '../handlers/multi_shibala.js');
let code = fs.readFileSync(filePath, 'utf8');

// 1. Instantiation
code = code.replace(
    /const activeTables = new Map\(\);/,
    `const MultiGameEngine = require('../services/multiGameEngine');\nconst engine = new MultiGameEngine('shibala', '十八仔', 1);`
);
...
async function openTable(replyToken, context) {
    ...
    if (tableState) {
        await sendTableFlex(replyToken, tableState, '【 十八仔賭局已開啟 】\\n請大家下注後，莊家可點擊開始骰牌！\\n(1分鐘未開始將自動取消)');
    }
}
...
console.log('Refactored multi_shibala.js');

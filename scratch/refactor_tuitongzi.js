...
const filePath = path.join(__dirname, '../handlers/multi_tuitongzi.js');
let code = fs.readFileSync(filePath, 'utf8');

// 1. Instantiation
code = code.replace(
    /const activeTables = new Map\(\);/,
    `const MultiGameEngine = require('../services/multiGameEngine');\nconst engine = new MultiGameEngine('tuitongzi', '推筒子', 1);`
);
...
async function openTable(replyToken, context) {
    ...
    if (tableState) {
        await sendTableFlex(replyToken, tableState, '【 推筒子牌局已建立 】\\n請大家下注後，莊家可點擊發牌開始！\\n(1分鐘未開始將自動取消)');
    }
}
...
console.log('Refactored multi_tuitongzi.js');

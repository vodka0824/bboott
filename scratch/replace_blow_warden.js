...
const replacement = `async function handleBlowWarden(replyToken, context) {
...
const startIndex = content.indexOf('async function handleBlowWarden(replyToken, context) {');
let endStr = '} catch (e) {\n        console.error(\'[Jail] handleBlowWarden Error:\', e);\n    }\n}';
const endIndex = content.indexOf(endStr, startIndex);

if (startIndex !== -1 && endIndex !== -1) {
    content = content.substring(0, startIndex) + replacement + content.substring(endIndex + endStr.length);
    fs.writeFileSync('services/jailLifeService.js', content, 'utf8');
    console.log('Regex replace successful');
} else {
    console.log('Regex replace failed. Target not found.');

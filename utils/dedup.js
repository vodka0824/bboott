 ...
 * 使用混合策略：先查記憶體，再查 Firestore (Atomic Create)
 * 
...
async function acquireLock(eventId) {
    ...
    try {
        // 2. Firestore Atomic Create (Global Lock)
        // 使用 create()，如果文件已存在會拋出錯誤 (ALREADY_EXISTS)
        // 這保證了即使多個實例同時處理，也只有一個能成功
        ...
        if (error.code === 6) { // 6 = ALREADY_EXISTS
            logger.info(`[Dedup] Event ${eventId} hit Firestore lock (Duplicate)`);
            // 也更新記憶體，減少後續查詢
            memoryCache.set(eventId, Date.now());
            return false;
        }
